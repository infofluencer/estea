...
const links: {
  ...
  { href: '/urunler', label: 'Çaylarımız', internal: true },
  { href: 'https://market.esteacay.com/iletisim/', label: 'İletişim' },
...
export default function Navbar() {
  ...
  return (
    <header id="main-nav" className="fixed inset-x-0 top-0 z-50 bg-transparent text-cream">
      ...
      >
        <ul className="space-y-1 bg-ink/80 px-4 pt-1 pb-5 backdrop-blur-sm">
          {links.map((link) => (
            <li key={link.href}>
              {link.internal ? (
                <Link
